...
# 1. Page Config & Extended Premium UI Styling (Pixel-Perfect για image_89e697.png)
...
st.markdown("""
    ...
    /* Enterprise Workspace Cards (image_89e697.png look) */
    .workspace-card { 
    ...
    .workspace-title { 
        font-size: 13px; 
        font-weight: 700; 
        color: #a78bfa; /* Premium μωβ/μπλε τίτλος */
    ...
    /* Custom Input Styles για να κουμπώσουν με το image_89e697.png */
...
# Αρχικοποίηση session states για τα δύο ανεξάρτητα Audit Logs των Treasury Movements
...
try:
    ...
    # ==========================================
        ...
        # =========================================================================================
        # 3. 🏁 ΑΝΑΒΑΘΜΙΣΜΕΝΟ ΔΙΠΛΟ WORKFLOW: LOG MANUAL TREASURY MOVEMENT (CISA vs LISA)
        # =========================================================================================
        ...
        # Χρησιμοποιούμε 2 μεγάλα main columns για να σπάσει η οθόνη ακριβώς στη μέση (CISA αριστερά, LISA δεξιά)
        main_col_left, main_col_right = st.columns(2)
        
        # --- ΑΡΙΣΤΕΡΗ ΠΛΕΥΡΑ: CASH ISA WORKSPACE ---
        with main_col_left:
            ...
            # Εσωτερικό Grid 2 στηλών για τη φόρμα και το Log του CASH ISA
            ...
            with cisa_grid_left:
                ...
                if st.button("Commit Movement to Audit Log", key="btn_commit_cisa"):
                    st.session_state.cisa_movements.append({
                        "From": cisa_from,
                        "To": cisa_to,
                        "Amount": f"£{cisa_amount:,.2f}",
                        "Reason": cisa_reason
                    })
                    st.toast("CISA Audit Log Updated!", icon="💜")
            
            with cisa_grid_right:
                st.markdown("<p style='font-size: 12px; font-weight: 700; color: #9ca3af; margin-bottom: 12px;'>ACTIVE VARIANCE ADJUSTMENTS LOG</p>", unsafe_allow_html=True)
                if st.session_state.cisa_movements:
                    st.dataframe(pd.DataFrame(st.session_state.cisa_movements), use_container_width=True, hide_index=True)
                else:
                    st.caption("No movements committed for Cash ISA today.")
                    
            st.markdown("</div>", unsafe_allow_html=True)

        # --- ΔΕΞΙΑ ΠΛΕΥΡΑ: LIFETIME ISA WORKSPACE ---
        with main_col_right:
            ...
            # Εσωτερικό Grid 2 στηλών για τη φόρμα και το Log του LIFETIME ISA
            ...
            with lisa_grid_left:
                ...
                if st.button("Commit Movement to Audit Log", key="btn_commit_lisa"):
                    st.session_state.lisa_movements.append({
                        "From": lisa_from,
                        "To": lisa_to,
                        "Amount": f"£{lisa_amount:,.2f}",
                        "Reason": lisa_reason
                    })
                    st.toast("LISA Audit Log Updated!", icon="💜")
            
            with lisa_grid_right:
                st.markdown("<p style='font-size: 12px; font-weight: 700; color: #9ca3af; margin-bottom: 12px;'>ACTIVE VARIANCE ADJUSTMENTS LOG</p>", unsafe_allow_html=True)
                if st.session_state.lisa_movements:
                    st.dataframe(pd.DataFrame(st.session_state.lisa_movements), use_container_width=True, hide_index=True)
                else:
                    st.caption("No movements committed for Lifetime ISA today.")
